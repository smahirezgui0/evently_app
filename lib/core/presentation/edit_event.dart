...
class _EditEventState extends State<EditEvent> {
  ...
  void initState() {
    ...
    selectedTime = TimeOfDay(
      hour: int.parse(widget.event.eventTime.split(':')[0]),
      minute: int.parse(widget.event.eventTime.split(':')[1]),
    );

    // تعيين الفئة المختارة
    selectedIndexCate = eventTypes.indexWhere(
            (type) => type["title"] == widget.event.category
    );
    if (selectedIndexCate == -1) selectedIndexCate = 0;
  }
  ...
  Future<void> updateEvent() async {
    try {
      ...
      // الطريقة الصحيحة للتحديث في Supabase
      final response = await supabase
          .from('events')
          .update(updateData)
          .eq('id', widget.event.id);  // استخدام eq بدلاً من match
              ...
              child: Container(
                child: Stack(
                  ...
                  children: [
                    ...
                    Text("Edit Event", style: AppTextStyles.addEventText),
                  ],
                ),
              ),
            ),
            ...
            Padding(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: Text("Update Event", style: AppTextStyles.btnText),
                ),
              ),
            ),
          ],
